...
//echo "<meta http-equiv='refresh' content='5;url=viewirbreviewideasvoting.php'>";
	...
	if ($votetotal==$emptotal)
	{
		...
		for($i=1;$i<=$votetotal;$i++)
		{
			...
			elseif (($nocnt > $yescnt) && ($nocnt > $onholdcnt))
			{
				$curr_status = '261';
			...
			{
				//$curr_status = '271';
				//$cmt2='There is a Tie on the Voting for this idea, Idea for Operations Override';
				//$act2='On Hold by IRB';
				...
				//mysql_query($sqlqry3);
				$curr_status = '96';
				$cmt2='Voting response Tied, Idea for Operations Override';
				$act2='Idea marked for Operations Override';
				$sqlqry5="INSERT INTO `trans_master`(`req_no`, `reviewer_type`, `email_id`, `comments`, `action_taken`) VALUES ('$req_no','$reviewer_type','$email','$cmt2','$act2')";
				//echo $sqlqry3."<br/>";
				mysql_query($sqlqry5);
...
<form name="account" id="account" action="<?php echo $_SERVER['PHP_SELF'].'?req_no='.$req_no?>" method="POST" enctype="multipart/form-data">
	...
	<table id="skillview" width="100%">
		...
		for ($x = 1; $x <= $tot; $x++)
			...
			{ 
			?>
				<input type='button' onclick='window.open("showirbpanelistvoting.php?req_no=<?php echo $req_no; ?>","VotingDetails","width=550,height=170,left=150,top=200,toolbar=0,status=0,");' value='Check Voting Result' class="controltext" />
			<?php
			}
			}
		...
		?>
		/>&nbsp;&nbsp;<input type="button" name="irbcancel" class="controltext" value="Cancel" onclick="javascript:history.back();" /><input type="hidden" name="cscbufunded" value="<?php echo $bufunded;?>" />
