	...
	//echo $sql;
		...
		?>
					<!--<table border="0" width="100%" cellpadding="2" cellspacing="2">
						...
						{
						?>
							<td>IRB Offering Business Plan <br/> IRB Business Plan Financial</td><td><a href="<?php echo $drmurl;?>">View</a><br/>
							<a href="<?php echo $tkmurl;?>">View</a></td>
							<td colspan="2">Comments: &nbsp;&nbsp;&nbsp;<textarea name="cscprcomments" readonly class="viewcontroltext" rows="3" cols="50"><?php echo $row['Comments'];?></textarea></td>
						<?php
						}
						else
						{
						?>
							<td colspan="4">Comments: &nbsp;&nbsp;&nbsp;<textarea name="cscprcomments" readonly class="viewcontroltext" rows="3" cols="50"><?php echo $row['Comments'];?></textarea></td>
						<?php	
						}
						?>
						</tr>
						<tr>						
							<td colspan="6" align="right"><input type="hidden" name="cscprreqno" class="viewcontroltext" value="<?php echo $row['request_num'];?>" /><input type="button" name="prcommentdetails" class="controltext" value="View All Comments" <?php if ($cnt==0) {?> disabled <?php } ?> onclick="window.open('ViewAllComments.php?req_no='+<?php echo $req_no;?>,'_blank','toolbar=yes, location=yes, directories=no, status=no, menubar=yes, scrollbars=yes, resizable=no, copyhistory=yes, width=700, height=300');">
